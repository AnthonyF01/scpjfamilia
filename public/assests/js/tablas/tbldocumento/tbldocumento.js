...
function ajaxLoad(filename, content, action = '', message = '') {
    ...
    if (typeof filename !== 'undefined' && filename != '') {
        ...
        if (filename.indexOf('edit') != -1) {
            $('#parent').remove();
            $(".breadcrumb").append("<li id='parent' class='active'><a href='"+redirect+"'><i class='fa fa-folder-open'></i> Documentos</a></li><li id='li_edit' class='active'><i class='fa fa-edit'></i> Editar Documento</li>");
        }else if (filename.indexOf('create') != -1) {
            $('#parent').remove();
            $(".breadcrumb").append("<li id='parent' class='active'><a href='"+redirect+"'><i class='fa fa-folder-open'></i> Documentos</a></li><li id='li_create' class='active'><i class='fa fa-edit'></i> Crear Documento</li>");
        }else {
            $('#li_create').remove();
            $('#li_edit').remove();
            $('#parent').remove();
            $(".breadcrumb").append('<li id="parent" class="active"><i class="fa fa-gears"></i> Documentos</li>');
        }
    }else{
        $('#li_create').remove();
        $('#li_edit').remove();
