...
function ajaxLoad(filename, content, action = '', message = '') {
    ...
    if (typeof filename !== 'undefined' && filename != '') {
        var redirect = "javascript:ajaxLoad(\""+filename.split("tblmotivo")[0]+"tblmotivo"+"\")";
        if (filename.indexOf('edit') != -1) {
            $('#parent').remove();
            $(".breadcrumb").append("<li id='parent' class='active'><a href='"+redirect+"'><i class='fa fa-folder-open'></i> Motivos</a></li><li id='li_edit' class='active'><i class='fa fa-edit'></i> Editar Motivo</li>");
        }else if (filename.indexOf('create') != -1) {
            $('#parent').remove();
            $(".breadcrumb").append("<li id='parent' class='active'><a href='"+redirect+"'><i class='fa fa-folder-open'></i> Motivos</a></li><li id='li_create' class='active'><i class='fa fa-edit'></i> Crear Motivo</li>");
        }else {
            $('#li_create').remove();
            $('#li_edit').remove();
            $('#parent').remove();
...
$(document).on('submit', 'form#form_motivo', function (event) {
    event.preventDefault();
    var form = $(this);
