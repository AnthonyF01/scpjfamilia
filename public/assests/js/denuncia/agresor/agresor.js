...
function ajaxLoad(filename, content, action = '', message = '') {
    ...
    if (typeof filename !== 'undefined' && filename != '') {
        var redirect = "javascript:ajaxLoad(\""+filename.split("agresor")[0]+"agresor"+"\")";
        if (filename.indexOf('edit') != -1) {
            $('#parent').remove();
            $(".breadcrumb").append("<li id='parent' class='active'><a href='"+redirect+"'><i class='fa fa-user'></i> Agresores</a></li><li id='li_edit' class='active'><i class='fa fa-edit'></i> Editar Agresor</li>");
        }else if (filename.indexOf('create') != -1) {
            $('#parent').remove();
            $(".breadcrumb").append("<li id='parent' class='active'><a href='"+redirect+"'><i class='fa fa-user'></i> Agresores</a></li><li id='li_create' class='active'><i class='fa fa-edit'></i> Crear Agresor</li>");
        }else {
            $('#li_create').remove();
            $('#li_edit').remove();
            $('#parent').remove();
            $(".breadcrumb").append('<li id="parent" class="active"><i class="fa fa-gears"></i> Agresores</li>');
        }
    }else{
        $('#li_create').remove();
        $('#li_edit').remove();
...
$(document).on('submit', 'form#form_agresor', function (event) {
    event.preventDefault();
    var form = $(this);
