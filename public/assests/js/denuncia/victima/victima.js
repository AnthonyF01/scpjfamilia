...
function ajaxLoad(filename, content, action = '', message = '') {
    ...
    if (typeof filename !== 'undefined' && filename != '') {
        var redirect = "javascript:ajaxLoad(\""+filename.split("victima")[0]+"victima"+"\")";
        if (filename.indexOf('edit') != -1) {
            $('#parent').remove();
            $(".breadcrumb").append("<li id='parent' class='active'><a href='"+redirect+"'><i class='fa fa-user'></i> Victimas</a></li><li id='li_edit' class='active'><i class='fa fa-edit'></i> Editar Victima</li>");
        }else if (filename.indexOf('create') != -1) {
            $('#parent').remove();
            $(".breadcrumb").append("<li id='parent' class='active'><a href='"+redirect+"'><i class='fa fa-user'></i> Victimas</a></li><li id='li_create' class='active'><i class='fa fa-edit'></i> Crear Victima</li>");
        }else {
            $('#li_create').remove();
            $('#li_edit').remove();
            $('#parent').remove();
            $(".breadcrumb").append('<li id="parent" class="active"><i class="fa fa-gears"></i> Victimas</li>');
        }
    }else{
        $('#li_create').remove();
        $('#li_edit').remove();
...
$(document).on('submit', 'form#form_victima', function (event) {
    event.preventDefault();
    var form = $(this);
